...
- App 启动即开始发现。0.1 最初采用“上次成功设备是唯一自动候选”；首版打磨阶段已由 SPEC-001 的完整设备 Registry 与“上次成功、其余保存、新发现”串行连接队列取代。
- 已有连接时扫描不自动切换；首页无常驻设备列表或主动断开入口，用户从状态卡打开设备选择 Sheet 切换目标。停止扫描不影响连接。
- 设备支持左滑立即移除；移除当前设备时断开连接并清除匹配的保存记录，不自动接管到下一台，附近设备后续仍可再次发现。
...
- 状态机测试：本地网络/定位权限拒绝、定位拒绝时保持 GEO 连接、异常断线后清理连接状态并保留可重试端点、启动恢复上次设备且失败不顺延、无保存端点时不自动连接、已有连接时发现不切换、用户手动切换、发现保留已有端点、稳定身份去重、移除当前端点并清除保存记录。
...
- 首次定位权限允许/拒绝。
- 普通 Wi-Fi 自动发现。
- 冷启动自动扫描并恢复上一次成功连接的 FMO；恢复失败时其他设备只进入选择 Sheet。
- 手动 `fmo.local` 和 IPv4。
- 手动端点存在时再次发现不会清空；设备可移除，当前设备移除后连接与保存记录同时清除。
- 已连接时重新扫描不切换；点击其他设备可切换；首页不存在主动断开按钮。
...
2026-08-05，按当时确认的连接 UX 完成代码修订：App 启动扫描、首台仅自动连接一次、失败不顺延，并移除首页主动断开入口。通用 Simulator 构建与完整单元测试通过，用户随后完成真实 FMO 回归并确认没有问题，0.1 计划关闭。同日 0.3 首页 UX 评审进一步以“恢复上次成功设备 + 设备选择 Sheet”取代首台自动连接；该后续规则以当前产品/技术规格和设备模块文档为准。
