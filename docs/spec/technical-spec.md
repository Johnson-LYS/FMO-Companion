...
- `DashboardSnapshot`、局部降级和首页/锁屏投影基础可以先基于已有公开 GEO 能力开发；每个设备内部字段的接入门槛仍是公开、只读、已文档化的接口契约，且必须说明传输、鉴权、字段含义、单位、更新时间和错误语义。
...
- “当前服务器”、TX/RX、设备实时 QSO 数和盒子到服务器延迟只能来自正式状态接口。由坐标派生的 Maidenhead 必须继承源坐标的时间与可信度。
...
- `RecentVoiceActivity` 是独立的离散事件模型。公开 APRS `VOCAL` 帧只能证明某呼号近期触发过语音活动，不能推导“当前说话人”、通联对象、会话开始或结束；事件展示期限使用可注入时钟单独计算，不得复用协议重放窗口。
