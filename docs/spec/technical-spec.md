...
- `QSO`：ADR-0007 本地只读同步、设备隔离缓存、查询、网格地图与 ADIF。
...
- `DashboardSnapshot` 是唯一规范状态，字段至少包含类型化值、来源、`observedAt`、可信度和可用性。0.3 设备页只消费白名单投影，不自行拉取或解释数据。
...
- 初始完整状态读取成功后，连接中的设备页在前台默认每 3 秒只重新读取一次 `station/getCurrent`，用于观察在盒子或官方后台发生的外部服务器切换；不轮询 QSO、频率或其他状态。后台暂停轮询，恢复前台时立即读取一次完整状态；保留状态连接失效时允许只重建该链路并重试一次，但不得改变设备连接相位。刷新必须可取消、间隔可注入，并在设备切换时隔离旧结果。
- 当前服务器刷新失败只降级本地状态链路；GEO 和 `/events` 继续独立工作。下一轮刷新先重建完整白名单状态快照，成功后再恢复只读当前服务器刷新。ADR-0005 状态会话不发送服务器切换写命令，也不承诺后台固定刷新频率。
- ADR-0010 服务器选择使用独立 `FmoStationControlWebSocketClient`，严格串行 `getListRange/getPinnedList/getCurrent/setCurrent`。它不复用 ADR-0005 状态会话；目录不持久化或跨会话复用，每个 `getListRange` 分页校验成功后立即投影部分目录，再继续读取剩余分页和收藏。`setCurrent` 成功后必须回读相同 UID，目录分页、响应大小、总数、UID 与名称均有上限。
...
- ADR-0007 只允许独立 `FmoQSOReadClient` 发送 `qso/getList` 与 `qso/getDetail`；它不能复用或扩展为通用管理客户端，也不能触发备份、恢复、清空、签名/公钥、密钥重置或设备端 ADIF。
- `QSOListPage` 只解码 `count/page/pageSize/list` 与摘要的 `logId/timestamp/toCallsign/grid`；`QSODetail` 只解码 `logId/timestamp/fromCallsign/toCallsign/fromGrid/toGrid/freqHz/mode/relayName/relayAdmin/toComment`。未知字段丢弃，类型/范围错误拒绝当前对象。
- QSO 使用与 Dashboard 状态会话分离的 WebSocket；同一 QSO 会话没有请求 ID，必须严格串行请求。切换设备、离开页面或进入后台时取消未完成任务，旧会话响应不得写入新设备缓存。
- 自动同步触发点为：所选设备连接/切换成功、App 进入 active、QSO 页面出现，以及页面可见期间可注入的低频变化检查；同时提供显式刷新。任何调度都不宣称后台或固定周期。
- 首次或需要完整对账时按服务端回显页大小分页读取摘要。缓存主键为规范化设备稳定身份与 `logId` 的组合，并保存摘要指纹；分页总数、顺序或指纹异常时重新完整对账，不假设 `logId` 永久不复用。
- 列表摘要优先持久化并立即可查询；最近可见记录可优先串行取详情，其他详情由用户打开或 ADIF 导出时补齐。详情补齐必须可取消、有界且不阻塞 MainActor。
- 只有全部摘要页在同一会话中成功后，才以该完整快照删除设备缓存中已不存在的记录；任何超时、断线、取消或部分成功只合并已确认新增/更新，不删除旧缓存。
- 同步状态至少区分 `neverSynced/syncing/current/stale/partial/failed`，保存最后完整成功时间但不记录 QSO 内容到诊断。离线时可读缓存，不把缓存时间表述为设备在线状态。
- Maidenhead 网格只能投影为网格区域，不生成虚构精确坐标或轨迹；搜索和详情字段缺失时局部降级。
- ADIF 导出读取 App 的规范化 QSO 缓存；若选中范围存在未补齐详情，先执行可取消的串行补齐并显示进度。自动同步记录不具备完整 SQLite 快照签名，不进入“已验证/待检查”状态。

### 本地接收音频

- ADR-0009 只允许独立 `FmoLocalAudioClient` 在所选设备的 `/audio` 建立只接收 WebSocket；不能复用或扩展 ADR-0005 状态客户端，也不能发送任何业务帧。
- 二进制消息必须恰为 4480 字节，并按 8 kHz、16-bit little-endian、单声道有符号 PCM 解码为 2240 个样本。文本 `p` 是唯一允许忽略的保活消息；其他文本、消息类型或帧长失败关闭。
- `FmoAudioMonitorModel` 在静音状态仍计算瞬时波形；声音按钮只控制后续样本是否送入 `AVAudioEngine`。流与播放器都使用有界缓冲，优先丢弃旧帧而不是累积延迟。
- 首次连接或切换设备时声音重置为关闭；首页与横屏共享同一模型，进入/离开全屏及前后台切换不得重连、停止流或改变声音开关。设备断线或根页面销毁时取消流，临时断线重连保留用户选择。
...
- 0.4 的收藏呼号可在 0.6 消息功能中复用为常用联系人；设备收藏服务器不得与 SPEC-012 的自建服务器管理或认证状态合并。

## 网络

- Bonjour 使用 Network.framework，浏览 `_http._tcp` 并解析端点。
- App 启动时创建单一 Bonjour 扫描会话，并读取完整稳定端点 Registry；上次成功设备排第一，其余保存设备保持稳定顺序，不持久化带接口作用域的易变解析 IP。
- 自动连接按“上次成功、其余保存、新发现”串行执行；候选失败后顺延，队列与扫描均耗尽后才呈现总体失败。任一设备成功后关闭自动队列，后续发现只增量合并。删除当前设备结束连接并取消本轮自动队列，不自动接管到下一台。
...
- 本地接收音频使用独立 `URLSessionWebSocketTask`，地址为 `ws://<host>/audio`；握手 `Origin` 从同一端点派生为 `http://<host>`，生命周期和容错遵循 ADR-0009。
...
- `FMOV4NetworkStore` 的短期事件缓存采用时间与数量双上限：最近 24 小时且最多 200 条；每次处理新帧时先清理过期事件，FMO 网络页再从范围与类型过滤后的快照中最多投影 3 条。
- 公网 FMO 远控属于 0.6：它通过 APRS-IS 发送公开的 APRS message 控制帧，需要服务端接受的可发送登录、Keychain 中按目标隔离的 SECRET、持久化 Counter、Time Slot、HMAC 和 ACK。它与局域网设备 `/ws` 管理通道没有复用关系。
- 用户授权抓包中观察到的局域网 `/ws` 配置写操作仍受 ADR-0005 排除。App 继续通过官方 Web 页面提供这些管理能力；除非接口所有者发布正式写入契约并以新 ADR 评审，否则不得把观察到的写命令加入本地状态客户端。

### APRS 消息写会话（0.6）

- 0.6 仅在 `scenePhase == .active` 时建立服务端接受的可发送消息会话；进入 inactive/background 后取消接收、ACK 等待与自动重试。恢复前台后重新连接，但不自动补发旧消息。公开 PASSCODE 算法不是强身份认证，不得在模型或文案中表述为用户真实性验证。
- 写会话与已验收的 `pass -1 + u/APFMO4` 只读边界按 ADR-0006 保持协议隔离：共享 CRLF/TNC2 基础原语，但使用不同 transport 实例、登录、过滤器、状态机和重连任务。
- 消息历史使用 SwiftData；凭据不进入消息模型。会话按完整 APRS 地址区分，收藏仍按规范化基础呼号去重。删除会话只删除本地消息，不影响收藏、目录或网络事件。
- 消息状态机固定为 `draft → sending → awaitingAcknowledgement → acknowledged / unconfirmed`。连接中断、进入后台或重试耗尽只能得到 `unconfirmed`，不能推导对端未收到。
- 普通消息采用“首次发送 + 最多 2 次自动重试”；当前产品策略为 30 秒，可注入且不是协议常量。远控命令禁止自动重试。
- ACK 使用对端完整身份和消息 ID 精确关联。重复入站消息只持久化一次，但每次要求 ACK 的重复报文都可重新回复 ACK；未知、迟到或重复 ACK 不得改变其他消息。
- 外部消息在持久化前校验地址、字符集、正文长度、消息 ID、帧长与格式；拒绝结果类型化且不保留原始帧。草稿可保留，离线消息队列与静默补发明确排除。
- PASSCODE 计算器接收规范化基础呼号：转为大写并去除 `-SSID`，以公开 APRS-IS 算法确定性计算；SSID 不参与。计算值只在创建登录行时短暂存在，不写入 Keychain、UserDefaults、SwiftData、日志或诊断，并以公开已知向量覆盖大小写、不同 SSID 和基础呼号变化。
- APRS 身份变化时重新规范化、计算并重建写会话。服务端拒绝可发送登录时，只读 FMO 网络不受影响，消息与远控单独降级。客户端继续禁止调用盒子 `/ws getPasscode`；盒子管理字段与本地公开算法没有数据依赖。

### FMO 公网远控（0.6）

- 远控使用固定版本的官方 APRS Remote Control Tool/文档建立脱敏正反向量；在 Time Slot、Counter、HMAC 输入、截断、目标地址和 ACK 语义逐字节确认前，不实现推测性编解码。
- SECRET 按可信目标 FMO 身份隔离存入独立 Keychain 命名空间，不作为全 App 共享值；其配置界面属于“设备 → 远程控制 → 远控设置”，不得挂在 FMO 网络身份下。目标可以暂时不在局域网，但必须来自用户当前选择或此前可信保存的设备身份。
- Counter store 必须原子化：签名前预留并持久化，同一 Time Slot 单调递增；任何发送、断网或崩溃失败均不回滚已预留值。
- `NORMAL`、`STANDBY` 是明确的单次发送操作；`REBOOT` 在生成签名和预留 Counter 前先完成危险确认与 LocalAuthentication。认证取消或失败不消耗 Counter。
- 命令发送后只等待目标来源匹配的 `ACK,CONTROL`，不自动重发。官方资料未公开可按动作、Time Slot 或 Counter 精确关联的 ACK 字段，因此不得声称更细粒度关联；超时状态为 `unconfirmed`，用户文案说明设备可能已经执行。
- UI 只消费目标、命令影响、发送阶段与结果投影；Time Slot、Counter、HMAC、Keychain 和原始帧仅存在于安全实现、测试或脱敏内部诊断。
- APRS 远控与 ADR-0005 本地 `/ws` 只读客户端、ADR-0010 本地服务器控制客户端完全隔离；公网远控不得加入服务器切换、频率、音量、OTA 或其他未批准写操作。
...
- App 不注入 JavaScript、不检查 DOM、不复制网页登录态；只有 ADR-0005/0007 已固定的用户授权类型化只读接口可由原生客户端使用，其他官方页面行为不得映射。
...
- 10,000 条 QSO 摘要的分页同步、索引和按需详情补齐在现代 iPhone 上不阻塞 MainActor，可取消并提供进度反馈；不得一次无界并发请求设备。
