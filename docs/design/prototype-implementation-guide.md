...
- 0.3 设备卡固定为三个内容区域：无前置图标的大字号呼号，下方同行展示六位梅登黑德/过滤距离；无前置图标的当前服务器名称；单行本地动态事件。深色卡内主信息使用显式浅色前景，不继承系统文字色。呼号同行的设备选择按钮只显示单行“绿点 + 设备名 + 下箭头”，“已连接”仅保留为辅助功能语义。服务器名称与动态事件合并在一个大圆角容器中，内部不用横/竖分隔线或二次胶囊。QSO 日志数和 `getUserPhyFreq` 单一频率不投影到设备页；经纬度不进入卡片。延迟、管理员和在线/最大人数默认隐藏；管理员只有取得可靠字段时才条件出现，不得以 APRS、导入 QSO 或 GEO RTT 替代。
