...
界面至少显示设备端点、连接状态、盒子坐标、手机坐标、最后操作和诊断入口。诊断按 Wi-Fi、主机与 TCP 端口、官方 HTTP 后台、GEO WebSocket 的依赖顺序实时展示状态，并为首个失败提供可行动建议。
...
- UI 测试：诊断入口的整行命中、诊断页、手动地址流程，以及本地网络权限拒绝的说明与系统设置恢复入口。
- 状态机测试：本地网络/定位权限拒绝、定位拒绝时保持 GEO 连接、异常断线后清理连接状态并保留可重试端点。
