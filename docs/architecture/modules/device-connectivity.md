...
- App 首页出现时并行启动发现与上次设备恢复。最后一次完成 GEO 连接并成功回读坐标的稳定端点是本次启动唯一自动连接候选；恢复失败后继续合并扫描结果，但不会自动尝试其他设备。没有保存端点时只扫描，由用户选择。已有连接时发现只更新列表，发现失败也不会降级现有连接。用户点击当前设备是幂等操作，点击其他设备会取消尚未完成的恢复/旧连接并切换目标；首页不提供主动断开按钮。
...
- 启动时仅恢复上一次成功连接端点、恢复失败不顺延、没有保存端点时发现不自动连接、已有连接时发现不切换，以及用户显式选择其他设备时取消旧连接并切换。
- 官方管理与 QSO URL 的确定性构造、无设备/无效地址恢复提示，以及 `SFSafariViewController` 呈现入口。
- UI 自动化场景覆盖诊断入口整行命中、未连接会话与独立探测说明、首页无常驻设备列表、状态卡打开设备选择 Sheet、启动恢复上次设备、附近设备只留作手动切换、手动地址流程、已保存设备原生左滑删除，以及本地网络拒绝时的设置恢复入口。场景通过 Debug 依赖注入稳定复现，不改变生产服务组合。
