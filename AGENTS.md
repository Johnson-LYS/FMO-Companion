...
- Xcode Cloud 使用 `ci_scripts/ci_pre_xcodebuild.sh` 在 `analyze`、`archive`、`build` 与 `build-for-testing` 的 `xcodebuild` 紧前方将 `CI_BUILD_NUMBER` 写入工程，并校验 App 实际解析的 `CURRENT_PROJECT_VERSION`，确保设置页关于区块、构建产物与 App Store Connect 使用同一 Build。`test-without-building` 只复用先前构建产物且恢复环境可能没有源码，必须在访问本地化输入或工程前跳过。不得在 UI 中读取或展示 CI 环境变量。
