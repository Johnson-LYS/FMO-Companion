...
- 切换到不同 FMO 时创建空快照，避免设备之间复用旧数据；同一设备从后台恢复时使用 `beginReconnection()` 把现有字段转为 `stale` 并继续展示，直到各链路刷新为当前值。
...
- Dashboard 状态聚合不持有 WebSocket、Bonjour 或 APRS 传输；只依赖注入的讲话者位置存储协议恢复已知坐标。首页 QTH 与横屏位置投影读取同一位置缓存，横屏另读取已验证的 `FMOV4NetworkSnapshot`。首页和全屏视图仅消费 Audio 模块注入的同一个类型化音频监视模型，不接触 WebSocket 消息或 PCM 字节。
...
- 讲话触觉策略测试覆盖开始、结束、说话人直接切换、重复状态、断流及前台/标签/偏好门控；真机验收补充开始/结束强度与短间隔合并体感。
