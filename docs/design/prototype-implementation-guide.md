...
- 0.3 设备卡固定为三个内容区域：无前置图标的大字号呼号，下方同行展示六位梅登黑德/过滤距离；主题色实底、深色文字且右侧带上下切换符号的当前服务器入口；单行本地动态事件。深色卡内其他主信息使用显式浅色前景，不继承系统文字色。呼号同行只放全屏按钮；设备选择移至导航栏右侧，以“绿点 + 设备名 + 下箭头”打开现有 Sheet。服务器入口与动态事件合并在一个大圆角容器中，内部不用横/竖分隔线。QSO 日志数和 `getUserPhyFreq` 单一频率不投影到设备页；经纬度不进入卡片。延迟、管理员和在线/最大人数默认隐藏；管理员只有取得可靠字段时才条件出现，不得以 APRS、导入 QSO 或 GEO RTT 替代。
...
- 当前设备连接成功且 App 位于前台时，根层建立唯一 `/audio` 会话；首页与横屏声音按钮共享同一 `FmoAudioMonitorModel`。首次连接、恢复前台或切换设备默认关闭声音；进入或退出横屏不得重连、停止音频或改变按钮状态。会话内的 WebSocket 首次失败或瞬时断流必须自动重建，重连期间保留用户声音开关；App 不活跃、设备断线或切换设备时才取消整个会话并停止播放器。横屏左侧波形在声音关闭时也持续更新。
...
- App 只持久化 `FavoriteCallsign`，以规范化基础呼号唯一化；目录服务器可搜索和查看详情，但不显示 App 星标。消息联系人复用收藏呼号。
- 设备首页与横屏仪表盘的当前服务器名称打开同一个服务器选择内容；首页使用底部 Sheet，横屏使用 Hero 上方的居中模态层。“收藏服务器”完全来自所选 FMO 的 `getPinnedList`，“所有服务器”来自 `getListRange`，两者都不复制进 App 收藏模型。收藏分组始终固定在列表顶部，首次读取期间显示加载状态，读取完成后原位替换为收藏条目或空状态，避免分组延迟出现。每次打开都清空上次会话目录并重新读取；首个 `getListRange` 分页通过校验后立即显示，后续分页加载时保留已返回条目和进度，不使用持久化缓存或旧会话结果冒充实时目录。
- 设备收藏当前只读；没有已批准 pin/unpin 写契约时，不在 App 提供收藏编辑按钮。
